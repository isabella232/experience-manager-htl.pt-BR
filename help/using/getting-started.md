...
>[!NOTE]
>
>Para executar a maioria dos exemplos fornecidos nesta página, é possível usar um ambiente de execução ao vivo chamado [Read Eval Print Loop](https://github.com/Adobe-Marketing-Cloud/aem-htl-repl).
...
Mesmo dentro do mesmo componente, os arquivos HTL podem ser usados junto com JSPs e ESPs. O exemplo a seguir mostra em **linha 1** como incluir um arquivo HTL de um arquivo JSP e em **linha 2** como um arquivo JSP pode ser incluído de um arquivo HTL:
...
**O HTL suporta as bibliotecas de tags JSP?** - Não, mas como mostrado na seção  [Loading Client ](getting-started.md#loading-client-libraries) Library (Carregando bibliotecas de clientes), o  [modelo e as ](block-statements.md#template-call) chamadas exibem um padrão semelhante.

**Os recursos HTL podem ser estendidos em um projeto AEM?** Não, não podem. HTL tem poderosos mecanismos de extensão para reutilização da lógica - a [Use-API](getting-started.md#use-api-for-accessing-logic) - e de marcação (as instruções [template &amp; call](block-statements.md#template-call)), que podem ser usadas para modular o código de projetos.
...
## Conceitos fundamentais de HTL {#fundamental-concepts-of-htl}
...
Este é um primeiro exemplo, que pode ser contido como está em um arquivo **`template.html`**:
...
* **[Bloquear declarações](block-statements.md)**  - Para exibir condicionalmente a variável  **&lt;h1>** é usado um atributo de dados  [`data-sly-test`](block-statements.md#test) HTML5. HTL fornece vários atributos desse tipo, que permitem anexar o comportamento a qualquer elemento HTML, e todos recebem o prefixo `data-sly`.
...
resultará em algo como o seguinte HTML, mas somente se houver uma propriedade **`jcr:title`** e uma propriedade **`jcr:description`** definidas, e se nenhuma delas estiver vazia:
...
O exemplo a seguir mostra em **linha 1** um comentário HTL e em **linha 2** um comentário HTML:
...
As expressões só podem ser colocadas em texto HTML ou valores de atributo, mas não em nomes de elementos ou nomes de atributos, ou não seriam mais HTML válido. Para definir nomes de elementos dinamicamente, a instrução [`data-sly-element`](block-statements.md#element) pode ser usada nos elementos desejados e para definir nomes de atributos dinamicamente, mesmo configurando vários atributos de uma só vez, a instrução [`data-sly-attribute`](block-statements.md#attribute) pode ser usada.
...
Isso pode parecer uma grande restrição, no entanto, é uma restrição desejada, pois a Linguagem de modelo HTML não deve ser abusada para gerar saída que não seja HTML. A seção [Use-API para acessar a lógica](getting-started.md#use-api-for-accessing-logic) abaixo apresenta como a lógica adicional pode ser chamada a partir do modelo, que pode ser usada se for necessária para preparar saída complexa para esses contextos. Por exemplo, uma maneira fácil de enviar dados do back-end para um script front-end é ter a lógica do componente para gerar uma string JSON, que pode ser colocada em um atributo de dados com uma expressão HTL simples.
...
Para obter mais detalhes sobre como controlar o escape, consulte a seção [Contexto de Exibição de Idioma de Expressão](expression-language.md#display-context).
...
Consulte a seção [Trabalhando com modelos do lado do cliente](getting-started.md#working-with-client-side-templates) mais abaixo para ver um exemplo.
...
## Recursos gerais de HTL {#general-capabilities-of-htl}
...
E a seguir `logic.js` o arquivo JavaScript executado no servidor foi colocado ao lado dele:
...
* A seção da instrução [`data-sly-use`](block-statements.md#use) explica tudo o que pode ser feito com essa instrução.
* A [Use-API page](use-api.md) fornece algumas informações para ajudar a escolher entre escrever a lógica em Java ou em JavaScript.
...
Na maioria das linguagens de modelo, esse exemplo poderia criar uma vulnerabilidade de script entre sites (XSS), porque mesmo quando todas as variáveis são automaticamente escapadas por HTML, o atributo `href` ainda deve ter escape específico de URL. Essa omissão é um dos erros mais comuns, porque pode ser muito facilmente esquecida, e é difícil de detectar de forma automatizada.
...
Assumindo o seguinte arquivo `logic.js`:
...
Observe como os dois atributos foram escapados de forma diferente, porque o HTL sabe que os atributos `href` e `src` devem ser escapados para o contexto do URI. Além disso, se o URI começasse com **`javascript:`**, o atributo teria sido removido totalmente, a menos que o contexto tivesse sido explicitamente alterado para outra coisa.
...
Se o valor da propriedade `class` estiver vazio, o Linguagem de Modelo HTML removerá automaticamente o atributo `class` inteiro da saída.
...
Em HTL, as bibliotecas do cliente são carregadas por meio de um modelo auxiliar fornecido pela AEM, que pode ser acessado por meio de [`data-sly-use`](block-statements.md#use). Três modelos estão disponíveis neste arquivo, que pode ser chamado por meio de [`data-sly-call`](block-statements.md#template-call):
...
### Carregamento completo de várias bibliotecas de clientes ao mesmo tempo {#loading-multiple-client-libraries-fully-at-once}
...
A seção sobre as instruções [template &amp; call](block-statements.md#template-call) fornece mais detalhes sobre como declarar e chamar esses modelos funcionam.
...
Um caso especial, em que a técnica explicada na seção [Limitações de remoção de Contextos especiais](getting-started.md#lifting-limitations-of-special-contexts) pode ser legitimamente usada, é escrever modelos do lado do cliente (como Handlebars, por exemplo) que estão localizados nos elementos **script**. A razão pela qual essa técnica pode ser usada com segurança nesse caso é porque o elemento **script** então não contém JavaScript como pressuposto, mas outros elementos HTML. Eis um exemplo de como isso funcionaria:
...
Como mostrado acima, a marcação que será incluída no elemento **`script`** pode conter declarações de bloco HTL e as expressões não precisam fornecer contextos explícitos, pois o conteúdo do modelo Handlebars foi isolado em seu próprio arquivo. Além disso, este exemplo mostra como o servidor executou o HTL (como no elemento **`h2`**) pode ser misturado com uma linguagem de modelo executada do lado do cliente, como o Handlebars (mostrado no elemento **`h3`**).

Uma técnica mais moderna seria, no entanto, usar o elemento HTML **`template`** em vez disso, pois a vantagem seria que ele não precisaria isolar o conteúdo dos modelos em arquivos separados.
